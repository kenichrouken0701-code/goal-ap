...
export default function Home() {
  ...
  const resetForm = () => {
    ...
    if (activeTab === 'week') setCurrentWeeklyDays(generateInitialWeeklyDays());
    if (activeTab === 'month') {
  ...
  const refineGoalWithAi = () => {
    ...
    callAi(`目標：${currentGoal}`, "あなたは目標設定の専門コーチです。改善後の目標、不足している要素、今すぐやるべき行動（3つ）を日本語で出力してください。");
  };

  const decomposeGoalWithAi = () => {
    if (!currentGoal.trim()) return;
    callAi(`目標：${currentGoal}`, "長期目標を短期行動に分解してください。月ごとの目標、週ごとの行動、今日やること、を日本語で出力してください。");
  };

  const summarizeReflectionWithAi = () => {
    const content = activeTab === 'day' ? `良かったこと: ${currentGoodPoints}\nやり直せるなら: ${currentRedoPoints}` : currentReflection;
    if (!content.trim()) return;
    callAi(`振り返り：\n${content}`, "あなたは振り返り分析の専門家です。達成できたこと、うまくいかなかった原因、次の改善アクション（3つ）を日本語で出力してください。");
  };
  ...
  const generateDaySummary = () => {
    ...
    const prompt = `【日付】${currentDisplayDate}\n【目標】${currentGoal}\n【予定】\n${scheduleText}\n【振り返り】${currentOverallRating}\n良：${currentGoodPoints}\n改：${currentRedoPoints}`;
    callAi(prompt, "1日のまとめを日本語で出力してください。フォーマット：■日付/■目標/■スケジュール/■振り返り（達成度・良・改）", true);
  };

  const generateWeekSummary = () => {
    const daysText = currentWeeklyDays.map(d => `${d.day}：目標: ${d.goal || 'なし'}, 達成度: ${d.rating || 'なし'}`).join('\n');
    callAi(`【期間】${currentWeekRange}\n【目標】${currentGoal}\n【日別】\n${daysText}`, "1週間のまとめを日本語で出力してください。フォーマット：■期間/■目標/■達成状況/■分析/■来週アクション", true);
  };

  const generateMandalaSummary = () => {
    const subGoalsText = currentMandalaData.subGoals.map((sg, i) => sg.goal ? `${i + 1}. ${sg.goal}\n${sg.actions.filter(a => a).map(a => `・${a}`).join('\n')}` : '').filter(t => t).join('\n\n');
    callAi(`【最終目標】${currentMandalaData.centerGoal}\n【中目標と行動】\n${subGoalsText}`, "マンダラチャートのまとめを日本語で出力してください。フォーマット：■最終目標/■戦略（8要素）/■行動プラン", true);
  };

  return (
    <div className="min-h-screen pb-20 bg-[#f8f9fa] font-sans text-gray-900">
      <Head>
      ...
      <main className="max-w-7xl mx-auto px-4 mt-6">
        <div className="flex bg-gray-100 p-1 rounded-xl mb-8 max-w-3xl mx-auto shadow-inner">
              ...
              <div className="flex-1 w-full space-y-6">
                ...
                <section className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
                  {activeTab === 'day' && (
                    <div className="flex items-center justify-between mb-6 pb-4 border-b border-gray-100">
                      <div className="flex items-center gap-2 text-gray-700 font-semibold">
                        <Calendar className="w-5 h-5 text-blue-500" /> 日付
                      </div>
                      <div className="flex items-center gap-2">
                  ...
                  <div className="flex items-center gap-2 mb-4 text-gray-700 font-semibold">
                    <Target className="w-5 h-5 text-blue-500" /> {PERIOD_LABELS[activeTab]}の目標
                  </div>
                ...
                {activeTab === 'week' && (
                  <section className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm overflow-x-auto">
                    <div className="grid grid-cols-7 gap-2 min-w-[600px]">
                      ...
                      {currentWeeklyDays.map((d, idx) => (
                        <div key={idx} className="space-y-2 p-2 bg-white border border-gray-100 rounded shadow-sm">
                          <textarea value={d.goal} onChange={(e) => updateWeeklyDay(idx, 'goal', e.target.value)} placeholder="目標" className="w-full text-[10px] p-1 bg-gray-50 rounded outline-none h-12 resize-none" />
                  ...
                  <section className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
                    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
                      {currentMonthlyData.map((m, idx) => (
                        <div key={m.month} className="p-3 bg-white border border-gray-100 rounded-xl shadow-sm space-y-2">
                ...
                {activeTab === 'mandala' && (
                  <section className="bg-white p-4 rounded-2xl border border-gray-100 shadow-sm overflow-x-auto">
                ...
                <section className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
                  <div className="flex items-center gap-2 mb-4 text-gray-700 font-semibold">
                    <MessageSquare className="w-5 h-5 text-green-500" /> 振り返り
                  </div>
                  {activeTab === 'day' ? (
                  ...
                  <button onClick={summarizeReflectionWithAi} disabled={isAiLoading} className="mt-4 flex items-center gap-2 px-4 py-2 bg-green-50 text-green-600 rounded-lg text-sm font-medium hover:bg-green-100 disabled:opacity-50 transition-colors">
            ...
            <motion.div key="history" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-4">
              ...
              {filteredEntries.length === 0 ? (
                ...
                filteredEntries.map(entry => (
                  <div key={entry.id} className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm hover:border-blue-200 transition-colors">
